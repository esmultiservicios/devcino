...
$(document).ready(pagination(1));
 $(function(){
	  ...
	  $('#nuevo-registro').on('click',function(e){
	   ...
	   $('#form_main_users #bs-regis').on('keyup',function(){
		  pagination(1);
       });
	   
	   $('#form_main_users #status').on('change',function(){
		  pagination(1);
       });	   
	   clean();
});
...
function pagination(partida){
	var dato = $('#form_main_users #bs-regis').val();
	var status_valor = "";
	
	if($('#form_main_users #status').val() == "" || $('#form_main_users #status').val() == null){
		status_valor = 1;
	}else{
		status_valor = $('#form_main_users #status').val();
	}
...
function getEstatus(){
	...
	$.ajax({
	    ...
	    async: true,
        success: function(data){		
		    $('#form_main_users #status').html("");
			$('#form_main_users #status').html(data);		
		}			
     });		
}
...
$('#form_main_users #reporte').on('click', function(e){
    e.preventDefault();
    reporteEXCEL();
});
