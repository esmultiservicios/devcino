...
$(document).ready(pagination(1));puesto();empresa();servicio();pagination_jornada_colaboradores(1);pagination_servicio(1);pagination_puestos(1);puestoServcioColaborador();getJornadaColaborador();getEstatus();
  $(function(){
	  ...
	  $('#nuevo-registro-colaborador-servicios').on('click',function(e){
	   ...
	   $('#form_main_colaboradores #bs-regis').on('keyup',function(){
		  pagination(1);
   	      return false;
       });

	   $('#form_main_colaboradores #status').on('change',function(){
		  pagination(1);
   	      return false;
       });
...
function pagination(partida){
	...
	var dato = $('#form_main_colaboradores #bs-regis').val();
	var estatus;
	
	if($('#form_main_colaboradores #status').val() == "" || $('#form_main_colaboradores #status').val() == null){
		estatus = 1;
	}else{
		estatus = $('#form_main_colaboradores #status').val();
	}
...
function getEstatus(){
	...
	$.ajax({
	    ...
	    async: true,
        success: function(data){		
		    $('#form_main_colaboradores #status').html("");
			$('#form_main_colaboradores #status').html(data);
...
$('#form_main_colaboradores #reporte_excel').on('click', function(e){
    e.preventDefault();
    reporteEXCEL();
});
</script>