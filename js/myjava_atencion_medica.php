		...
		if(getAtencionPaciente(agenda_id) == 0){ 			  
			...
			swal({
			  ...
			  if (inputValue === "") {
				...
				marcarAtencion(agenda_id, inputValue);
			});		
	   }else{	
			swal({
				title: "Error", 
...
$('#end_atencion').on('click', function(e){
	...
	swal({
		...
		marcarAtencion($('#formulario_atenciones #agenda_id').val(), inputValue);
	});	
});
...
function viewExpediente(pacientes_id){
	...
	$.ajax({
		...
		success: function(valores){
			...
			$('#formulario_atenciones #expediente_alergias_respuesta').val(datos[23]);
