...
$(document).ready(function() {
	  $('#form_main_preclinica #nuevo_registro').on('click',function(e){
	   e.preventDefault();
	   if (getUsuarioSistema() == 1 || getUsuarioSistema() == 2 || getUsuarioSistema() == 3 || getUsuarioSistema() == 4 || getUsuarioSistema() == 6){
        ...
        $('#formulario_agregar_preclinica #fecha').val($('#form_main_preclinica #fecha_i').val());
    ...
    //BUSQUEDA	
	$('#form_main_preclinica #servicio').on('change',function(){	
	     $('#form_main_preclinica #colaborador').html("");	
	     pagination(1);
    });	
	
	$('#form_main_preclinica #colaborador').on('change',function(){     	    
		pagination(1);
    });		
	
	$('#form_main_preclinica #bs-regis').on('keyup',function(){
	   pagination(1);
    });

    $('#form_main_preclinica #fecha_i').on('change',function(){
	   pagination(1);
    });	
	
    $('#form_main_preclinica #fecha_f').on('change',function(){
	   pagination(1);
    });		
...
function pagination(partida){
	...
	var dato = '';
	var unidad = '';
	var fechai = $('#form_main_preclinica #fecha_i').val();
	var fechaf = $('#form_main_preclinica #fecha_f').val();	
	var colaborador = '';
	
	if($('#form_main_preclinica #unidad').val() == "" || $('#form_main_preclinica #unidad').val() == null){
		unidad = "";
	}else{
		unidad = $('#form_main_preclinica #unidad').val();
	}
	
	if($('#form_main_preclinica #colaborador').val() == "" || $('#form_main_preclinica #colaborador').val() == null){
		colaborador = '';
	}else{
		colaborador = $('#form_main_preclinica #colaborador').val();
	}
	
	if($('#form_main_preclinica #bs-regis').val() == "" || $('#form_main_preclinica #bs-regis').val() == null){
		dato = '';
	}else{
		dato = $('#form_main_preclinica #bs-regis').val();
	}
...
function getServicioFormMain(){
	...
	$.ajax({
	    ...
	    async: true,
        success: function(data){	
		    $('#form_main_preclinica #servicio').html("");
			$('#form_main_preclinica #servicio').html(data);
		}			
     });		
}
...
$(document).ready(function() {
	  $('#form_main_preclinica #servicio').on('change', function(){
		var servicio_id = $('#form_main_preclinica #servicio').val();
		...
		$.ajax({
            ...
            success: function(data){
				$('#form_main_preclinica #unidad').html("");
				$('#form_main_preclinica #unidad').html(data);				
            }
         });
...
function getProfesionales(){
	...
	$.ajax({
        ...
        success: function(data){
		    $('#form_main_preclinica #colaborador').html("");
			$('#form_main_preclinica #colaborador').html(data);			
		}			
     });	
}	
...
function eliminarRegistro(id, comentario){
	...
	var fecha = $('#form_main_preclinica #fecha_i').val();	
