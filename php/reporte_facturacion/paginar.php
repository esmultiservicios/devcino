...
if($type == 1 || $type == 2 || $type == 4){//SUPER ADMINISTRAOD, ADMINISTRADOR Y CONTADOR GENERAL
	...
	if($profesional != ""){
		$where = "WHERE f.fecha BETWEEN '$fechai' AND '$fechaf' AND f.colaborador_id = '$profesional' AND f.estado ".$in." AND f.usuario = '$colaborador_id'";
	}else if($dato != ""){
		$where = "WHERE f.estado ".$in." AND f.usuario = '$colaborador_id' AND (CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%' OR f.number LIKE '$dato%')";
	}else{
		$where = "WHERE f.fecha BETWEEN '$fechai' AND '$fechaf' AND f.estado ".$in." AND f.usuario = '$colaborador_id'";
	}
}
