...
$queryAtencion = "SELECT CONCAT(co.nombre, ' ',co.apellido) AS 'profesional', CONCAT(p.nombre, ' ',p.apellido) AS 'cliente', p.fecha_nacimiento AS 'fecha_nacimiento', p.email AS 'email', p.telefono1 AS 'telefono', at.*, s.nombre AS 'servicio'
	FROM atenciones_nutricion AS at
	INNER JOIN pacientes AS p
	ON at.pacientes_id = p.pacientes_id
	INNER JOIN colaboradores AS co
	ON at.colaborador_id = co.colaborador_id
	INNER JOIN servicios AS s
	ON at.servicio_id = s.servicio_id		
	WHERE at.pacientes_id = '$pacientes_id'";	
