...
$registro = "SELECT pre.preoperacion_id AS 'preoperacion_id',  DATE_FORMAT(pre.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.nombre,' ',p.apellido) AS 'paciente', p.identidad AS 'identidad', CONCAT(co.nombre,' ',co.apellido) AS 'colaborador', s.nombre AS 'servicio', (CASE WHEN p.genero = 'H' THEN 'Hombre' ELSE 'Mujer' END) AS 'sexo',
(CASE WHEN pre.paciente = 'N' THEN 'N' ELSE 'S' END) AS 'paciente_tipo', pre.talla AS 'talla', pre.peso_actual As 'peso_actual', pre.imc_actual AS 'imc_actual', pre.resultados AS 'resultados', pre.fecha_cirugia AS 'fecha_cirugia',
(CASE WHEN pre.paciente = 'N' THEN 'X' ELSE '' END) AS 'n',
(CASE WHEN pre.paciente = 'S' THEN 'X' ELSE '' END) AS 's',
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("L$fila", $registro2['talla']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("M$fila", $registro2['peso_actual']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("N$fila", $registro2['imc_actual']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("O$fila", $registro2['resultados']);
